function [struct_b1] = simulation_loop_b1(initial_batteryLevel, harvestedEnergy, trafficProfile, energy_prices, battery_max_level, refThreshold, days_simulation, hours_simulation, plotting)
...
for day=1:days_simulation
   ...
   for hour=1:hours_simulation
       ...
       indexes = array_batteryLevel_BS < refThreshold;

       % Acquiring data
       % Purchased energy
       array_purchased_energy(day,hour) = sum(refThreshold - array_batteryLevel_BS(indexes));
       
       % Cost
       array_cost(day,hour) = sum(refThreshold - array_batteryLevel_BS(indexes))*array_SG_prices(hour);
       
       % Battery level update after trading
       array_batteryLevel_BS(indexes) = refThreshold;
